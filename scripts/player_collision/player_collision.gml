...
function player_collision()
{
	...
	if(!ground)
	{
		...
		var c = collision_active_sensor(hitbox_w, hitbox_h, COLLISION_MODE.FLOOR, plane, semi_solid_condition);
		...
		// Ceiling collison
		c = collision_active_sensor(hitbox_w, hitbox_h, COLLISION_MODE.CEILING, plane);	
	...
	else
	{
		...
		if(PLAYER_ALT_COLLISION_MODE)
		{
			...
			var col = collision_get_distance(px, py, (mode + COLLISION_MODE.LEFT_WALL) % 4, plane, true);
			var a = collision_get_angle(px, py, (mode + COLLISION_MODE.LEFT_WALL) % 4, plane);
